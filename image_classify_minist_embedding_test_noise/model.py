...
class model(object):
    def __init__(self,sess,config,logging):
        ...
        self.img_embed_dim=CF.config["img_embed_dim"]
...
#                self.image_batch=tf.nn.conv2d(self.image_batch_i, w_input, strides=[1, 1, 1, 1], padding='VALID')
    def _input(self):
        self.image_batch_i= tf.placeholder(tf.int32, shape=[None,self.hight, self.width])
        self.image_label_batch=tf.placeholder(tf.int32, shape=[None,self.n_classes])
        self.image_batch=self.image_batch_i
    ...
    def build(self):
        ...
        if self.model_type=="1":
            ...
            self.alex_net(self.image_batch, self.keep_prob)
...
############################################################################################################################
    def structure_1(self):
        with tf.variable_scope('conv1') as scope:
            self.W = tf.Variable(
                tf.random_uniform([CF.config['max_']-CF.config['min_']+1, self.img_embed_dim], -1.0, 1.0),
                name="W")
            
            self.img_embedding = tf.nn.embedding_lookup(self.W, self.image_batch)
            
            print(self.img_embedding)
            self.image_batch = self.img_embedding
            #[None,h,w,16]
            input_dim = self.img_embedding.get_shape()[-1].value
            weights = tf.Variable(tf.truncated_normal(shape=[3, 3, input_dim, 64], stddev=1.0, dtype=tf.float32),
...
#############################################################################################################################
    ...
    def structure_2(self):
        #image=[B,28,28]
        self.W = tf.Variable(
            tf.random_uniform([256, self.img_embed_dim], -1.0, 1.0),
            name="W")
        self.img_embedding = tf.nn.embedding_lookup(self.W, self.image_batch)
        #print(self.img_embedding)
        self.image_batch = self.img_embedding
        ...
        with tf.name_scope('conv1_layer'):
            w_conv1 =self.weight_variable([3, 3, input_dim, 64], 64, use_l2=False, lam=0)
...
########################################################################################################################################
    ...
    def alex_net(self,x,dropout):
        #[None,h,w]
        self.W = tf.Variable(
            tf.random_uniform([256, self.img_embed_dim], -1.0, 1.0),
            name="W")
        self.img_embedding = tf.nn.embedding_lookup(self.W, x)
        #print(self.img_embedding)
        x = self.img_embedding
        input_dim = x.get_shape()[-1].value
        weights = {
            'wc1': tf.Variable(tf.random_normal([7, 7, input_dim, 96])),  # [None,]
...
#################################################################################################################################
    def get_inception_layer(self, inputs, conv11_size, conv33_11_size, conv33_size,
        ...
        with tf.variable_scope("conv_1x1"):
            conv11 = layers.conv2d( inputs, conv11_size, [ 1, 1 ],stride = 1 )
            
        with tf.variable_scope("conv_3x3"):
            conv33_11 = layers.conv2d( inputs, conv33_11_size, [ 1, 1 ],stride = 1 )
            conv33 = layers.conv2d( conv33_11, conv33_size, [ 3, 3 ],stride = 1 )
            
        with tf.variable_scope("conv_5x5"):
            conv55_11 = layers.conv2d( inputs, conv55_11_size, [ 1, 1 ],stride = 1 )
            conv55 = layers.conv2d( conv55_11, conv55_size, [ 5, 5 ],stride = 1 )
            
        with tf.variable_scope("pool_proj"):
            pool_proj = layers.max_pool2d( inputs, [ 3, 3 ], stride = 1 )
            pool11 = layers.conv2d( pool_proj, pool11_size, [ 1, 1 ] ,stride = 1)
    ...
    def aux_logit_layer(self,inputs, n_classes, is_training ):        
        ...
        with tf.variable_scope("conv11"):
            conv11 = layers.conv2d( pooled, 128, [1, 1],stride = 1 )
    ...
    def googlenet(self,inputs):
        self.W = tf.Variable(
            tf.random_uniform([256, self.img_embed_dim], -1.0, 1.0),
            name="W")
        self.img_embedding = tf.nn.embedding_lookup(self.W, inputs)
        #imput_dim = self.img_embedding.get_shape()[-1].value
        inputs = self.img_embedding
        with tf.name_scope( "google_net", "googlenet", [inputs] ):
            with ops.arg_scope( [ layers.max_pool2d ], padding = 'SAME' ):
                ...
                pool0 = layers.max_pool2d(conv0, [3, 3], stride=1, scope='pool0')
                conv1_a = layers.conv2d( pool0, 64, [ 1, 1 ], stride = 1, scope = 'conv1_a' )
                conv1_b = layers.conv2d( conv1_a, 192, [ 3, 3 ], stride = 1, scope = 'conv1_b' )
                pool1 = layers.max_pool2d(conv1_b, [ 3, 3 ], stride=1,scope='pool1')
                ...
                pool2 = layers.max_pool2d(inception_3b, [ 3, 3 ],stride=1, scope='pool2')
                ...
                pool3 = layers.max_pool2d(inception_4e, [ 3, 3 ], stride=1, scope='pool3')
                ...
                reshape = tf.reshape( pool4, [-1, 1024*27*27] )
