...
if is_train:
    if is_test:
        ...
        val=input_data.get_test_files(t_label2id, file_dir="noise_test")
    else:
        tra,t_label2id,n_class=input_data.get_train_files(file_dir="train")
...
if is_train:
    for k in range(max_step*total_num):
        ...
        #st=time.time()
            batch_x,batch_y=feature_extra.give_batch(train_lst,pix_x,pix_y,ref=False,convert_RGB=False)
            batch_x = Normalize(batch_x,CF.config['max_'],CF.config['min_'])
        ...
        #st=time.time()
            ...
            if (step+1)%step_to_test==0:
                if CF.config["num"] in ["vgg16","Alexnet"]:
                    ...
                    batch_x,batch_y=feature_extra.give_batch(valvgg,pix_x,pix_y,ref=False,convert_RGB=False)
                    batch_x = Normalize(batch_x,CF.config['max_'],CF.config['min_'])
                else:
#                    print(val)
                    batch_x,batch_y=feature_extra.give_batch(val,pix_x,pix_y,ref=False,convert_RGB=False)
                    #print(batch_x)
                    batch_x = Normalize(batch_x,CF.config['max_'],CF.config['min_'])
 #               print(batch_x)
